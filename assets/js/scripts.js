...
updateRecentList();
$(".btn-stop").click(stopSong);
$(".fm-range-input").on("input",function(){
    $(".fm-herts").html($(this).val());
})
}
...
function getItemUI(item){
    ...
    $template.find(".fa-play").click(function(){
        startSong(item.path);
    });
...
function deleteItem(item){
    $.ajax({
        type: "POST",
        url: config.getApiPath("/delete.php"),
        data:item,
        dataType:"json",
        success: function (data) {
            showToastMessage(data.message);
...
function startSong(songName){
    var ch = $(".fm-range-input").val();
    $.ajax({
        type: "POST",
        url: config.getApiPath("/radio.php"),
        data:{
            request:"start",
            song:songName,
            ch:ch
